    ...
    <section id="squad" style={{padding:"52px 0 60px",background:"#FDFAF6",overflow:"hidden"}}>
      <style>{`
        ...
        @media (max-width:768px) {
          .squad-title { font-size: 20px !important; }
          .squad-strip { font-size: 10px !important; }
        }
      `}</style>
      ...
      <div style={{position:"relative"}}>
        <div style={{position:"absolute",left:0,top:0,bottom:0,width:60,background:"linear-gradient(to right,#FDFAF6,transparent)",zIndex:10,pointerEvents:"none"}}/>
        <div style={{position:"absolute",right:0,top:0,bottom:0,width:60,background:"linear-gradient(to left,#FDFAF6,transparent)",zIndex:10,pointerEvents:"none"}}/>
        ...
        >
          {TRACK.map((c,i)=>{
            ...
            return (
              <div key={i} onClick={()=>setActive(i % SQUAD.length)} style={{
                cursor:"pointer", flex:`0 0 ${CARD_W}px`, width:CARD_W,
                border:`1px solid ${isActive?"#4CAF50":"rgba(26,95,122,.2)"}`,
                overflow:"hidden", background:"#fff",
                boxShadow:isActive?"0 6px 24px rgba(76,175,80,.2)":"0 2px 8px rgba(0,0,0,.05)",
                transform:isActive?"translateY(-6px)":"translateY(0)",
                transition:"transform .25s,box-shadow .25s,border-color .25s",
              }}>
                <div style={{width:"100%",paddingTop:"125%",position:"relative",overflow:"hidden",background:"#ddd4c0"}}>
                  <div id={`letter-${i}`} style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Cinzel',serif",fontSize:40,fontWeight:700,color:"#1a5f7a",opacity:0,transition:"opacity .2s",zIndex:0}}>{c.name[0]}</div>
                  <img
                    id={`img-${i}`}
                  ...
                  />
                  {isActive && <div style={{position:"absolute",inset:0,background:"rgba(76,175,80,.15)",zIndex:2,pointerEvents:"none"}}/>}
                </div>
      ...
      <div className="squad-strip" style={{textAlign:"center",marginTop:16,padding:"0 20px"}}>
        <span style={{fontFamily:"'Cinzel',serif",fontSize:16,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:"#1a5f7a"}}>{SQUAD[active].name}</span>
