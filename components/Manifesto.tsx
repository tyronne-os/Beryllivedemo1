...
export default function Manifesto() {
  return (
    <section style={{background:"#0D1117",padding:"56px 20px",position:"relative",overflow:"hidden",textAlign:"center"}}>
      ...
      <div style={{maxWidth:640,margin:"0 auto",position:"relative"}}>
        ...
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(232,220,200,.75)",marginBottom:24,fontFamily:"'Cormorant Garamond',serif"}}>
          Every character in the Beryl Squad is a living digital soul — not software. Trained to listen, respond, and connect at a level that text can never reach.
        </p>
        <div style={{marginBottom:28}}>
          {["Sub-180ms Latency","RunwayML Powered","Voice Cloning","12 Characters","Emotion-Aware"].map(tag=>(
            <span key={tag} style={{display:"inline-block",fontFamily:"'Cinzel',serif",fontSize:16,fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase",padding:"5px 12px",border:"1px solid rgba(76,175,80,.4)",color:"#4CAF50",margin:4}}>{tag}</span>
          ))}
        </div>
        <Link href="/demo">
