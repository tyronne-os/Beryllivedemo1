# BERYL CLIQUE — THE COMMERCIAL
### Working title: *"It Was Supposed to Be Easier"*
**Runtime:** ~95 seconds · 8 scenes · Runway Seedance (≤15s each)
**Purpose:** Plays on load at berylize.com. Built to close investors.
**Format:** 16:9, cinematic, photorealistic. Color arc: cold blue isolation → warm gold belonging.

---

## 🎯 THE BIG IDEA

> **AI was supposed to make things easier. Instead it gave us more code, more configs, more dread.**
>
> Beryl Clique throws all of it away. **No keyboard. No chat box.** Just an old-fashioned,
> face-to-face conversation with co-workers — who, somewhere along the way, became your friends.

That's the whole film. The villain isn't a competitor — **we never name one.** The villain is the
*feeling* of building with AI today: the glow of an editor at 2 AM, red errors, the quiet despair of
"wait… wasn't this supposed to be simpler?" The hero is the most human thing imaginable — **a real
conversation with people you like.** That contrast is the sale.

---

## 🎯 THE EMOTIONAL ARC

| Act | Scenes | Feeling | Color |
|-----|--------|---------|-------|
| **I — The Dread** | 1–3 | "AI was supposed to help. This is agony." | Cold teal/blue, 2 AM |
| **II — The Turn** | 4–5 | The keyboard dissolves. Warmth enters. | Blue → gold bloom |
| **III — The Friendship** | 6–8 | Face to face with friends. The reveal. | Full royal gold |

The throughline motif: **No keyboard. No chat box. Just conversation.** In Act II we literally watch
the keyboard and the text-input box dissolve into gold light — replaced by faces.

---

## 🎙 VOICE-OVER SCRIPT (single narrator — warm, male, confident, unhurried)

> *Casting note: the calm conviction of an Apple/Mercedes spot. Not hype. Lots of air between lines —
> let the picture breathe. The first half is weary; the second half quietly comes alive.*

| # | Timecode | Line |
|---|----------|------|
| 1 | 0:00–0:07 | *(quiet, tired)* "They told us AI would make things… easier." |
| 2 | 0:08–0:15 | *(dry)* "Instead — we got *this*." |
| 3 | 0:16–0:24 | "Endless code. Errors that argue back. Another framework to learn… at 2 AM." |
| 4 | 0:25–0:31 | *(a beat — almost a whisper)* "Wasn't this supposed to be *simpler*?" |
| 5 | 0:32–0:38 | *(the turn — hope enters)* "So what if you never wrote another line?" |
| 6 | 0:39–0:47 | "No keyboard. No chat box. No prompts to perfect." |
| 7 | 0:48–0:56 | "Just a conversation. Face to face. With your team." |
| 8 | 0:57–1:07 | "Meet your Clique. Co-workers who know your name, remember your wins, and show up — *live* — every day." |
| 9 | 1:08–1:16 | *(warm, intimate)* "Co-workers who, somewhere along the way… became friends." |
| 10 | 1:17–1:24 | "Every one of them — powered by Beryl Live Human OS." |
| 11 | 1:25–1:33 | "Our pipeline? Orchestrated by **gstack** — open-sourced by the CEO of Y Combinator himself." |
| 12 | 1:34–1:39 | *(final)* "AI was never supposed to feel like work." |
| 13 | 1:40–1:43 | "It was supposed to feel like *this*." |
| 14 | 1:44–1:48 | *(logo)* "Beryl Clique. berylize.com." |

---

## 🎵 SOUND DESIGN

- **0:00–0:24** — Sparse, melancholy solo piano, one repeating note. Diegetic: keyboard clacks
  (deliberately a little too loud, a little oppressive), a frustrated exhale, a clock ticking, distant city hum.
- **0:25–0:38** — Piano suspends on an unresolved chord. The clacking stops. A held silence on *"simpler?"* — then a single warm note blooms on *"what if."*
- **0:39–1:07** — Strings enter and build. The keyboard sounds are gone for good. In their place: the warm
  ambient murmur of a real meeting — light laughter, a few friendly voices.
- **1:08–1:33** — Full, elegant orchestral swell + subtle choir. Triumphant but never bombastic.
- **1:34–1:48** — Drops to one warm sustained chord under the logo. A single gold "shimmer" chime on the wordmark.

---

## 🎬 THE SCENES — COPY-PASTE SEEDANCE PROMPTS

> **CRITICAL — CHARACTER CONSISTENCY:** Generate ONE reference still of the protagonist first (see
> "Pre-Production"), then use **image-to-video** with that still for scenes 1, 2, 4, 5, 6. Same face
> throughout. Scenes 3, 7, 8 are abstract/graphic and don't need him.
>
> **DO NOT name or show any real product/brand/logo** in the editor — generic, anonymous code only.
> The enemy is the *feeling*, never a competitor.
>
> **Protagonist = "MARCUS":** man, early 30s, warm brown skin, short cropped dark hair, neat light-beard
> stubble, charcoal-gray henley. Relatable, intelligent, everyman founder/developer.
>
> **THE AGENTS' LOOK (Scenes 5, 6, 7) — IMPORTANT:** Every agent wears a **matching navy-blue branded
> blazer** (this is the Clique uniform / brand identity — consistency across the team). But each one is
> calling in from a **different real-world remote-work location**, like an authentic work-from-home team:
> one in a cozy living room, one in a home office with bookshelves, one on a sunny apartment balcony, one
> in a stylish hotel room, one in a bright kitchen, one in a modern co-working loft. Same blazer, different
> worlds — it reads instantly as *real people on a real video call.*

---

### SCENE 1 · "The Dread" — 15s
**VO:** Lines 1–2 · **Mood:** AI was supposed to help — this is agony

```
Cinematic over-the-shoulder shot of a man in his early 30s with short cropped dark hair and light beard
stubble, wearing a charcoal-gray henley, hunched tensely over a laptop in a dim home office late at night.
On the laptop screen: a generic code editor packed wall-to-wall with dense, anonymous programming code and
glowing red error underlines, a terminal below spilling endless error messages. No recognizable logos or
brands. Cold blue-white screen light is the only illumination on his weary face and the dark cluttered room.
Empty coffee cups, scattered sticky notes. The camera slowly pushes in over his shoulder toward the
oppressive glowing wall of code. Shallow depth of field, 35mm anamorphic lens, faint volumetric haze, moody
teal-and-blue color grade, fine film grain. Atmosphere of isolation, dread, and late-night frustration.
16:9, photorealistic, ultra-detailed.
```

---

### SCENE 2 · "Wasn't This Supposed to Be Easier" — 12s
**VO:** Line 3 · **Mood:** burnout

```
Cinematic medium close-up of the same early-30s man with short dark hair and light beard, side-lit by the
cold glow of a laptop in a dark room. He leans back slowly, closes his eyes, pinches the bridge of his nose,
then drags a hand down his tired face and exhales in defeat. The faint reflection of scrolling code and red
errors glows in his glasses. A wall clock behind him reads 2:00 AM. Subtle, naturalistic movement. Soft
side lighting, deep shadows, teal-blue color grade, shallow depth of field, 50mm portrait lens, cinematic
film grain. Mood: quiet burnout and disillusionment. 16:9, photorealistic.
```

---

### SCENE 3 · "Drowning" — 12s
**VO:** *(no VO — held breath + music)* · **Mood:** the irony of complexity

```
Abstract cinematic shot flying through a dark digital void filled with floating, tangled translucent code —
endless anonymous lines of programming, glowing red error messages, knotted node-and-wire diagrams, and
configuration files multiplying and overlapping chaotically. No brands or logos. Cold blue and red light.
The camera pushes forward through the suffocating tangle as it grows denser and closes in, overwhelming the
frame. Volumetric fog, lens flares, depth of field, dark moody cyber color grade, drifting particles.
Feeling of complexity spiraling out of control — the opposite of the simplicity that was promised. 16:9,
photorealistic, high detail.
```

---

### SCENE 4 · "The Turn" — 12s
**VO:** Lines 4–5 · **Mood:** hope ignites

```
Cinematic over-the-shoulder shot of the same early-30s man in the dark office, pausing. He stops typing and
sits still. Then, slowly, he closes the wall of code and types a single clean web address into an empty
browser bar: "berylize.com". As he presses enter, a warm golden light begins to bloom from the center of the
screen, gently pushing back the cold blue of the room and rising onto his face. His exhausted expression
softens into curiosity and hope. Slow, deliberate camera. The color grade visibly shifts from cold teal
toward warm gold within the shot. Shallow depth of field, 35mm lens, gentle volumetric light rays, cinematic
film grain. Mood: a quiet turning point. 16:9, photorealistic.
```

---

### SCENE 5 · "The Keyboard Dissolves" — 15s
**VO:** Lines 6–7 · **Mood:** liberation — *the signature shot*

```
Cinematic shot of the same early-30s man bathed in growing warm golden light. On the desk before him, the
computer keyboard and a floating on-screen text chat box slowly dissolve into drifting particles of gold
light and vanish. As they disappear, the cold code on his screen melts away and reorganizes into an elegant
grid of warm, living human faces — a team of diverse, smiling professional people, each wearing a matching
navy-blue branded blazer but clearly calling in from a different real-world location: one from a cozy living
room, one from a home office with bookshelves, one from a sunny apartment balcony, one from a stylish hotel
room. He lifts his hands off the vanished keyboard, sits upright in quiet awe, the reflection of the smiling
faces glowing in his eyes. Gold particles drift through the warming room. Camera gently arcs from the empty
desk up to his awestruck face. Warm gold-and-amber color grade, soft bokeh, 50mm lens, cinematic film grain,
dreamlike. Mood: liberation and wonder — the keyboard is gone, replaced by people. 16:9, photorealistic.
```

---

### SCENE 6 · "Face to Face with Friends" — 15s
**VO:** Lines 8–9 · **Mood:** warmth, genuine friendship

```
Cinematic wide shot of the same early-30s man sitting back comfortably and relaxed in a warm modern space —
no keyboard, no devices in sight — surrounded by a floating semicircle of glowing video panels, each showing
a different diverse, professional AI co-worker. Every co-worker wears the same matching navy-blue branded
blazer, but each is calling in from a distinctly different real-world remote-work setting: one from a cozy
living room sofa, one from a home office lined with bookshelves, one from a sunlit apartment balcony with
plants, one from a stylish hotel room, one from a bright modern kitchen, one from a co-working loft. They are
smiling, laughing, gesturing, leaning in, genuinely engaged like close friends catching up. He is animated
and at ease, talking with his hands, laughing along, fully in his element among friends. Warm golden ambient
light, real human warmth and chemistry, subtle floating gold particles, premium dark-and-gold production
design. Slow elegant camera move drifting around the circle of friendly faces in their different worlds. Rich
amber color grade, anamorphic lens flares, shallow depth of field, cinematic film grain. Mood: friendship,
ease, belonging — a remote team of co-workers who became friends. 16:9, photorealistic, ultra-detailed.
```

---

### SCENE 7 · "The Reveal" — 12s
**VO:** Lines 10–11 · **Mood:** credibility, scale

```
Cinematic shot pulling back further to reveal the entire warm glowing room and the full circle of friendly AI
co-worker panels orbiting the man like a constellation — each agent in a matching navy-blue branded blazer,
each in their own different remote location (living rooms, home offices, balconies, hotel rooms) — settling
into an elegant grid. Golden light intensifies.
A refined gold heraldic shield emblem with the letters "B · O · S" elegantly forms and glows at the center of
the frame above the team, with subtle royal-gold particle effects. Deep luxurious black-and-gold palette,
volumetric god rays, slow majestic camera, cinematic film grain, premium and aspirational. Mood: revelation,
prestige, scale. 16:9, photorealistic, high detail.
```

---

### SCENE 8 · "The Close" — 12s
**VO:** Lines 12–14 · **Mood:** iconic, final

```
Cinematic minimalist logo reveal on a deep black background with a subtle texture of royal purple velvet and
faint gold fleur-de-lis patterns at the edges. In the center, an elegant shimmering gold wordmark "BERYL
CLIQUE" draws itself in with a soft metallic gold sweep of light, refined serif typography. Beneath it,
smaller gold text glows: "berylize.com". A single gold light flares across the wordmark and fades. Slow,
still, premium, luxurious. Black-and-gold, gentle volumetric glow, cinematic. Mood: iconic, timeless, final.
16:9, photorealistic.
```

---

## 🛠 PRE-PRODUCTION — DO THIS FIRST

### 1. Generate the "Marcus" reference still
So the same face appears in every shot. Use Runway gen4_image (same pipeline that made Cleo):

```
Photorealistic portrait of a relatable man in his early 30s, warm brown skin, short cropped dark hair, neat
light beard stubble, wearing a charcoal-gray henley shirt, intelligent thoughtful expression, seated at a
desk, soft cinematic lighting, neutral dark background, 50mm lens, shallow depth of field. 16:9,
ultra-photorealistic.
```
→ Then feed this still into **image-to-video** for Scenes 1, 2, 4, 5, 6.

### 2. Generate order
Scenes 1 → 8 in sequence; each is independent, so render in parallel if credits allow.

### 3. Assembly
- Lay all 8 clips on a timeline in order.
- Crossfade Scene 3→4 and 5→6 (the two emotional pivots) with a ~0.5s dissolve.
- The **keyboard-dissolve in Scene 5** is the visual money-shot — give it room; don't cut away early.
- Lay the VO on top, then the music bed under it. Kill all keyboard SFX the instant Scene 5 begins.
- Gold "shimmer" chime exactly on the Scene 8 wordmark draw.

### 4. Web delivery
- Export **1080p H.264 MP4**, target < 8 MB (it autoplays muted on load — compress hard).
- Also export a **WebM** for smaller size.
- Save to `public/videos/clique-commercial.mp4`.
- Tell me the filename and I'll wire it into the hero as:
  ```jsx
  <video autoPlay muted loop playsInline poster="/characters/CLEO_SHIELD.png" />
  ```
  with a "tap for sound" control and a graceful fallback to the current static hero.

---

## 📋 SHOT LIST — QUICK REFERENCE

| # | Title | Len | Needs Marcus? | Render type |
|---|-------|-----|---------------|-------------|
| 1 | The Dread | 15s | ✅ | img→video |
| 2 | Wasn't This Supposed to Be Easier | 12s | ✅ | img→video |
| 3 | Drowning | 12s | ❌ | text→video |
| 4 | The Turn | 12s | ✅ | img→video |
| 5 | The Keyboard Dissolves | 15s | ✅ | img→video |
| 6 | Face to Face with Friends | 15s | ✅ | img→video |
| 7 | The Reveal | 12s | ❌ | text→video |
| 8 | The Close | 12s | ❌ | text→video |

**Total: ~105s of source → cut to ~95s final.**
